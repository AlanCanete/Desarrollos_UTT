...
def ultimasemana():
    ...
    return f"{semanaTrabajo:02}", agnotrabajo,lunes_ant,domingo_ant

def Parametros():
    today = datetime.datetime.today()
    for_4_weeks = today - datetime.timedelta(weeks=4)
    for_4_mondays = for_4_weeks.weekday()
    monday = for_4_weeks - datetime.timedelta(days=for_4_mondays)

    day_for_sunday = (today.weekday() +1)%7
    last_sunday = today - datetime.timedelta(days=day_for_sunday)
...
nameFile = 'PRECIOS_COLUN_'

try:
    #lunes,domingo = Parametros()
    semana,agno,lunes, domingo = ultimasemana()
    lunesf=int(lunes.strftime("%Y%m%d"))
    domingof=int(domingo.strftime("%Y%m%d"))
    ...
    print(lunesf,domingof)
    #cursor.execute(query, (lunesf,domingof))
    #columns = [column[0] for column in cursor.description]
    #with open(f"{destino}{nameFile}{agno}{semana}.csv",'w', newline='',encoding='utf-8') as csvfile:
