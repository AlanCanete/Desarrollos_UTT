...
def UploadFileSftp(rutaOrigen, rutaDestino):
    ...
    rutasftp = f"/Semanales/Ecommerce cecinas/{rutaDestino}"
    try:
...
try:
    ...
    query = query + ",sum ([ventaconvertida]) as Vta_Kg,[empresa],[categoria],[fabricante],[marca],[char1] as Segmento,[char2] as Tipo,[char5] as Familia,[char6] as Subsegmento"
    query = query + ",[char7] as Tier,[char8] as Envasado_Granel,[char9] as Gramaje,[premium_masivo_cial],[canal],[cadena] FROM [CLIENTES_CIAL].[dbo].[FIAMBRES_KEYACCOUNT_SEMANAL_CIAL_CON_TPR]"
    query = query + "WITH(NOLOCK) where agno_conv_wmrt >= '2022' and canal = 'ONLINE' group by [agno_conv_wmrt],[semana_conv_wmrt],[name_semana_conv_wmrt],[upc],[descri_item],[empresa],[categoria]"
    query = query + ",[fabricante],[marca],[char1],[char2],[char5],[char6],[char7],[char8],[char9],[premium_masivo_cial],[canal],[cadena]"
