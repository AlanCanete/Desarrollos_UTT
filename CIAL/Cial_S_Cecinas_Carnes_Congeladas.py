...
nameFile = 'S Cecinas-Carnes Congeladas'
...
try:
    ...
    query = "SELECT * FROM [CLIENTES_CIAL].[dbo].[TBLCecinasCarnesCongeladas] WITH(NOLOCK)"
    cursor.execute(query)
    columns =[column[0] for column in cursor.description]

    with open(f"{destino}{nameFile} - TD (US).csv",'w',newline='',encoding='utf-8')as csvfile:
        writer = csv.writer(csvfile, delimiter=";")
        writer.writerow(columns)
        for row in cursor.fetchall():
            writer.writerow(row)

    archivo=f"{nameFile} - TD (US).csv"
